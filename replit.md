...
- **Backend filtering**: GET `/api/competitions?clubId=xxx` and `/api/opposition-teams?clubId=xxx` return only that club's data
- **Frontend updates**: All pages and dialogs (fixtures, settings, analysis, videos, etc.) pass current club's ID when fetching/creating competitions and opponents
- **Data migration**: Existing competitions and opponents assigned to their respective clubs; shared opponents duplicated for each club
- **getOrCreateCompetition/getOrCreateOppositionTeam**: Updated to match by name+clubId and create with clubId
